...
sliderBtn.addEventListener('click', function () {
  ...
  const duration = document.getElementById('duration').value || 1000;
	...
	if (duration >= 1000) {
		...
		alert("You have to set the time minimum 1000ms (1 second).");
	}
});

document.querySelector('#search').addEventListener('keypress', function (enter) {
	if (enter.key === 'Enter') {
		document.querySelector('.main').style.display = 'none';
		clearInterval(timer);
		const search = document.getElementById('search');
...
function selectAll() {
	document.getElementById('allSelect').addEventListener('change', function (e) {
